...
`E[(S + T)⁴] = E[S⁴] + 6 E[S²] E[T²] + E[T⁴]` for independent centred `S`, `T`, and handles the
random coefficients of the induction on `j` by independence, without conditional expectations.
...
/-! ### Toolkit: bounded measurable functions -/
...
/-! ### Toolkit: monomials in two independent factors -/
...
/-- Second moments add, one independent centred summand at a time:
`E[(S + T)²] = E[S²] + E[T²]`. -/
...
/-- The triangle inequality along a `Finset`: a partial sum inherits an almost-sure bound. -/
...
/-! #### Almost-sure boundedness, closed under the operations used below -/
...
-- a σ-algebra built for the arguments below; kept semireducible
...
/-! #### The three term shapes that occur -/
...
lemma integral_sq_sum_mul_background_le [IsProbabilityMeasure μ]
  ...
  | @insert a F ha ih =>
    ...
    -- integrability of the five functions the expansion names
...
/-- **Cauchy--Schwarz** for two almost surely bounded measurable functions:
`E[fg] ≤ (E[f²])^{1/2}(E[g²])^{1/2}`. No sign condition is needed. -/
...
By induction on `j`: write `Z = ∑_{i ∈ I_0} ξ^{(0)}_i Zᵢ` with
...
/-- **`j = 0`.** The empty product is `1`, `Z = a` is nonrandom, and the bound reads
...
/-- `integral_pow_four_random_le` with a random coefficient: the coefficient is the first fair
sign, the summand the second, and the background σ-algebra the one the first generates. -/
